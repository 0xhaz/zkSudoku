...
template Sudoku() {
   ...
   for (var i = 0; i < 9; i++) {
    ...
    // If unsolved[i][j] is not 0, it means that solved[i][j] is equal to unsolved[i][j]
    // If unsolved[i][j] is 0, it means that solved[i][j] is different from unsolved[i][j]
    ...
    // element is not equal to previous elemtns in the same square
